...
void FRISCFrameLowering::emitPrologue(MachineFunction &MF,
                                      MachineBasicBlock &MBB) const {}

void FRISCFrameLowering::emitEpilogue(MachineFunction &MF,
                                      MachineBasicBlock &MBB) const {}

MachineBasicBlock::iterator
FRISCFrameLowering::eliminateCallFramePseudoInstr(MachineFunction &MF,
												MachineBasicBlock &MBB,
    											MachineBasicBlock::iterator I)
																		 const {
  const FRISCInstrInfo &TII =
      *static_cast<const FRISCInstrInfo *>(MF.getSubtarget().getInstrInfo());
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Amount != 0) {
      ...
      if (Old.getOpcode() == TII.getCallFrameSetupOpcode()) {
        ...
        Amount -= CalleeAmt;
        if (Amount)
          New = BuildMI(MF, Old.getDebugLoc(), TII.get(FRISC::ADD_ri),
                        FRISC::SP)
                    .addReg(FRISC::SP)
                    .addImm(Amount);
      }
...
bool
FRISCFrameLowering::spillCalleeSavedRegisters(MachineBasicBlock &MBB,
  ...
  for (unsigned i = CSI.size(); i != 0; --i) {
    ...
    MBB.addLiveIn(Reg);
    BuildMI(MBB, MI, DL, TII.get(FRISC::PUSH))
...
bool
FRISCFrameLowering::restoreCalleeSavedRegisters(MachineBasicBlock &MBB,
                                              MachineBasicBlock::iterator MI,
                                        const std::vector<CalleeSavedInfo> &CSI,
                                        const TargetRegisterInfo *TRI) const {
  if (CSI.empty())
