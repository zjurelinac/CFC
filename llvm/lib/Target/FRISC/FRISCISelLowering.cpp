//===-- FRISCISelLowering.cpp - FRISC DAG Lowering Implementation  ------------===//
//
...
FRISCTargetLowering::FRISCTargetLowering(const FRISCTargetMachine &TM, const FRISCSubtarget &STI)
    : TargetLowering(TM) {
    ...
    setStackPointerRegisterToSaveRestore(FRISC::SP);
...
SDValue FRISCTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
    switch (Op.getOpcode()) {
        ...
        default:
    	    DEBUG(dbgs() << "LowerOperation unhandled SDValue: "); Op.dumpr(); DEBUG(dbgs() << "\n");
            llvm_unreachable("unimplemented operand");
    }
}

// Calling Convention Implementation
#include "FRISCGenCallingConv.inc"

// Transform physical registers into virtual registers
SDValue FRISCTargetLowering::LowerFormalArguments(SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    ...
    CCInfo.AnalyzeFormalArguments(Ins, CC_FRISC);
...
SDValue FRISCTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    ...
    CCInfo.AnalyzeReturn(Outs, RetCC_FRISC);
    ...
    return DAG.getNode(FRISCISD::RET_FLAG, DL, MVT::Other, RetOps);
}

static SDValue EmitCMP(SDValue &LHS, SDValue &RHS, SDValue &TargetCC, ISD::CondCode CC,
        const SDLoc &dl, SelectionDAG &DAG) {

    assert(!LHS.getValueType().isFloatingPoint() && "We don't handle FP yet");

    FRISCCC::CondCodes TCC = FRISCCC::COND_INVALID;
    ...
    switch (CC) {
        ...
        case ISD::SETGE:
            ...
            if (const ConstantSDNode * C = dyn_cast<ConstantSDNode>(LHS)) {
                LHS = RHS;
                RHS = DAG.getConstant(C->getSExtValue() + 1, dl, C->getValueType(0));
                TCC = FRISCCC::COND_SLT;
                break;
            }
            TCC = FRISCCC::COND_SGE;
            break;
        case ISD::SETGT:
            if (const ConstantSDNode * C = dyn_cast<ConstantSDNode>(LHS)) {
                LHS = RHS;
    ...
    return DAG.getNode(FRISCISD::CMP, dl, MVT::Glue, LHS, RHS);
}

SDValue FRISCTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
    ...
    return DAG.getNode(FRISCISD::BR_CC, dl, Op.getValueType(), Chain, Dest, TargetCC, Flag);
}

SDValue FRISCTargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
...
SDValue FRISCTargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG &DAG) const {
    ...
    return DAG.getNode(FRISCISD::Wrapper, SDLoc(Op), PtrVT, Result);
}

SDValue FRISCTargetLowering::LowerExternalSymbol(SDValue Op, SelectionDAG &DAG) const {
    SDLoc dl(Op);
...
MachineBasicBlock * FRISCTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI, MachineBasicBlock *BB) const {
    ...
    if (Opc == FRISC::Select) {
        ...
        BuildMI(BB, dl, TII.get(FRISC::JPcc_i)).addMBB(copy1MBB).addImm(MI.getOperand(3).getImm());
        ...
        if (Opc == FRISC::MUL_rr)
            BuildMI(*BB, I, dl, TII.get(FRISC::CALL_i)).addExternalSymbol("__frisc__mul");
        else if (Opc == FRISC::UDIV_rr)
            BuildMI(*BB, I, dl, TII.get(FRISC::CALL_i)).addExternalSymbol("__frisc__udiv");
        else if (Opc == FRISC::SDIV_rr)
            BuildMI(*BB, I, dl, TII.get(FRISC::CALL_i)).addExternalSymbol("__frisc__sdiv");
        else
...
SDValue FRISCTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI, SmallVectorImpl<SDValue> &InVals) const {
    ...
    // FRISC target does not yet support tail call optimization.
    isTailCall = false;
    ...
    for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
        ...
        if (VA.isRegLoc()) {
            ...
            if (!StackPtr.getNode())
                StackPtr = DAG.getCopyFromReg(Chain, dl, FRISC::SP, PtrVT);
            ...
            if (Flags.isByVal()) {
                ...
                MemOp = DAG.getMemcpy(Chain, dl, PtrOff, Arg, SizeNode, Flags.getByValAlign(),
    ...
    // Build a sequence of copy-to-reg nodes chained together with token chain and flag operands which copy
    // the outgoing args into registers.  The InFlag in necessary since all emitted instructions must be stuck together.
    SDValue InFlag;
    ...
    Chain = DAG.getNode(FRISCISD::CALL, dl, NodeTys, Ops);
    InFlag = Chain.getValue(1);
...
static void AnalyzeReturnValues(CCState &State, SmallVectorImpl<CCValAssign> &RVLocs, const SmallVectorImpl<ArgT> &Args) {
    ...
    // Reverse splitted return values to get the "big endian" format required to agree with the calling convention ABI.
...
//
SDValue FRISCTargetLowering::LowerCallResult(SDValue Chain, SDValue InFlag, CallingConv::ID CallConv, bool isVarArg,
