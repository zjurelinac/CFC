...
FRISCTargetLowering::FRISCTargetLowering(const FRISCTargetMachine &TM,
                                     const FRISCSubtarget &STI)
    : TargetLowering(TM) {
...
SDValue FRISCTargetLowering::LowerFormalArguments(
...
SDValue
FRISCTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
...
static SDValue EmitCMP(SDValue &LHS, SDValue &RHS, SDValue &TargetCC,
  ...
  case ISD::SETTRUE:
    ...
    break;
  case ISD::SETEQ:
    TCC = FRISCCC::COND_Z;     // aka COND_Z or COND_EQ
    ...
    break;
  case ISD::SETNE:
    TCC = FRISCCC::COND_NZ;    // aka COND_NZ or COND_NE
    // Minor optimization: if LHS is a constant, swap operands, then the
    // constant can be folded into comparison.
    if (LHS.getOpcode() == ISD::Constant) {
      std::swap(LHS, RHS);
...
SDValue FRISCTargetLowering::LowerExternalSymbol(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc dl(Op);
...
static void AnalyzeArguments(CCState &State,
  ...
  static const MCPhysReg RegList[] = {
    FRISC::R24, FRISC::R25, FRISC::R26, FRISC::R27, FRISC::R28, FRISC::R29, FRISC::R30,
    FRISC::R31
  };
  ...
  for (unsigned i = 0, e = ArgsParts.size(); i != e; i++) {
    ...
    if (!UseStack && Parts <= RegsLeft) {
      ...
      UseStack = true;
      for (unsigned j = 0; j < Parts; j++)
        CC_FRISC(ValNo++, ArgVT, LocVT, LocInfo, ArgFlags, State);
    }
  }
}*/

SDValue FRISCTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
...
///
SDValue FRISCTargetLowering::LowerCallResult(
